...
<div class="bigidea">
  ...
  function</strong> with millions of adjustable numbers — its <strong>parameters</strong> (you'll also hear
  them called its <strong>weights</strong>): the dials of the function — that, given some text so far,
  predicts what comes next. <em>Training</em> tunes those dials (millions of tiny nudges) until coherent
  language falls out.
...
&ldquo;a 3-million-parameter model&rdquo; (ours) just means 3 million dials, and a frontier model has hundreds
of billions. The dials are turned only once, during training (§7) — frozen afterwards; the single knob you
