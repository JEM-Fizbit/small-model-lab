...
<div class="bigidea">
  ...
  function</strong> with millions of adjustable numbers — its <strong>parameters</strong>, also called its
  <strong>weights</strong>: the dials of the function — that, given some text so far, predicts what comes
  next. <em>Training</em> tunes those dials (millions of tiny nudges) until coherent language falls out.
...
<p>Those numbers are the function's <strong>dials</strong>, and you'll meet them under two interchangeable
names: a model's <strong>parameters</strong> and its <strong>weights</strong> are the same thing — each is
one adjustable dial. Set all of them together the right way and the output goes from gibberish to fluent
English; that one good setting <em>is</em> the trained model. So &ldquo;a 3-million-parameter model&rdquo;
(ours) just means 3 million dials, and a frontier model has hundreds of billions. The dials are turned only
once, during training (§7) — frozen afterwards; the single knob you actually touch at generation is
temperature (§8).</p>
"""),
 ],
},
...
<p>A token doesn't <em>have</em> parameters. The parameters — the <strong>weights</strong>, every
<code>W</code> and <code>b</code> — <strong>are the model</strong>: fixed after training and
<strong>shared</strong>, so the exact same weights process every token, at every position. A token is
just data flowing through them.</p>
