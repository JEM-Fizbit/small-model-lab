...
end you'll understand, concretely, what people mean when they say a model
&ldquo;predicts the next word.&rdquo;</p>

<div class="bigidea">
  <p><strong>The whole idea, in one sentence:</strong> a GPT is a machine that, given some
  text so far, guesses what comes next — and <em>training</em> is the process of making
  those guesses less wrong, millions of times, until coherent language falls out.</p>
  <p>Everything below is just unpacking that sentence: how text becomes numbers, how the
...
in a loop and the machine writes.</p>
...
small <strong>MLP</strong> — a two-layer mini-network applied to each position on its own,
giving the model room to &ldquo;think&rdquo; about what attention just collected. A
<strong>block</strong> is simply: attention, then MLP. A GPT is just this block stacked
<code>n_layer</code> times.</p>
...
letter at a time and trained for four minutes. It proves the whole machine works end to end;
