...
# ---------------------------------------------------------------- DIAGRAMS --
DISTILL_SVG = r'''<svg viewBox="0 0 820 210" role="img" aria-label="Distillation flow: oncology trials are read by the Claude Sonnet 5 teacher, which writes one gold JSON label per trial, which fine-tune the Qwen3-4B student model.">
...
<text x="304" y="80" text-anchor="middle" font-size="13.5" font-weight="700" fill="#2c4a52">Claude Sonnet 5</text>
...
<text x="514" y="118" text-anchor="middle" font-size="10.5" fill="#6e6557">one per trial</text>
<!-- box 4: student -->
...
model (Claude Sonnet 5) reads each trial and writes the gold answer, and the small student then learns
...
<tr><td>intervention_class</td><td>0.827</td><td>0.640</td><td>0.933</td><td>0.944</td><td>+0.304</td><td>0.944</td></tr>
<tr><td>modalities (set-F1)</td><td>0.330</td><td>0.695</td><td>0.859</td><td>0.862</td><td>+0.167</td><td>0.862</td></tr>
<tr><td>primary_endpoint_type</td><td>0.380</td><td>0.640</td><td>0.933</td><td>0.936</td><td>+0.296</td><td>0.936</td></tr>
...
<tr><td>risk flags (set-F1)</td><td>0.643</td><td>0.662</td><td>0.794</td><td>0.831</td><td>+0.169</td><td>0.831</td></tr>
</tbody></table>

"""),
  ("prose", r"""
<p class="tablenote">In the <b>Shipped tool</b> column the three marked rows are no longer generated:
<span class="prov">read</span> is copied from the registry record, <span class="prov">computed</span>
is arithmetic on it. The four unmarked rows are what the fine-tuned model is actually asked for, and
they are identical to the <b>Qwen student</b> column &mdash; it is the same adapter.</p>
